...
class TestGetUserActiveTime:
    ...
    def test_single_activity_within_date_range(self, mock_db, init_users_and_teams):
    ...
    def test_activity_before_start_date_filtered_out(
    ...
    def test_activity_after_end_date_filtered_out(self, mock_db, init_users_and_teams):
    ...
    def test_timezone_conversion(self, mock_db, init_users_and_teams):
    ...
    def test_timezone_conversion_crosses_date_boundary(
        self, mock_db, init_users_and_teams
    ):
    ...
    def test_timezone_conversion_america_los_angeles_boundary(
        self, mock_db, init_users_and_teams
    ):
        ...
        assert result2[0].date == datetime.date(
            2023, 1, 15
        )  # Jan 15 in PST (7 UTC = 23 PST previous day)

    def test_moving_time_activity_types_use_moving_seconds(
        self, mock_db, init_users_and_teams
    ):
    ...
    def test_moving_time_activity_with_null_moving_seconds_uses_elapsed(
        self, mock_db, init_users_and_teams
    ):
    ...
    def test_non_moving_time_activity_uses_elapsed_seconds(
        self, mock_db, init_users_and_teams
    ):
    ...
    def test_multiple_activities_same_user_same_date_accumulated(
        self, mock_db, init_users_and_teams
    ):
    ...
    def test_multiple_activities_same_user_different_dates(
        self, mock_db, init_users_and_teams
    ):
        ...
        assert result[1].user.id == "user1"
        assert result[1].date == datetime.date(2023, 1, 16)
        assert result[1].active_seconds == 2400

    def test_multiple_users_same_date(self, mock_db, init_users_and_teams):
    ...
    def test_complex_scenario_multiple_users_dates_activities(
        self, mock_db, init_users_and_teams
    ):
    ...
    def test_activity_on_boundary_dates(self, mock_db, init_users_and_teams):
        ...
        activity_start.save(force_insert=True)
        ...
        activity_end.save(force_insert=True)
        config = ScoreConfig(
            score_start_date=datetime.date(2023, 1, 1),
            score_end_date=datetime.date(2023, 1, 31),
            time_zone="UTC",
